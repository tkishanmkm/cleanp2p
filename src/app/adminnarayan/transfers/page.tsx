    ...
    const fetchTransfers = async () => {
      ...
      try {
        ...
        const mapped: CoinTransfer[] = (data || []).map((t: any) => ({
          id: t.id,
          publicId: t.public_id || t.publicId || t.id,
          senderId: t.sender_id || t.senderId,
          senderUsername: t.sender_username || t.senderUsername || 'Sender',
          recipientId: t.recipient_id || t.recipientId,
          recipientUsername: t.recipient_username || t.recipientUsername || 'Recipient',
    ...
    <>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>View a log of all direct coin transfers between users.</CardDescription>
        ...
        <CardContent>
                ...
                {!isLoading &&
                  filteredTransfers?.map((transfer) => (
                    <TableRow key={transfer.id} onClick={() => handleRowClick(transfer)} className="cursor-pointer">
                      ...
                      <TableCell>
                        <Link
                          href={`/adminnarayan/users/${transfer.senderUsername || transfer.senderId}`}
                      ...
                      <TableCell>
                        <Link
                          href={`/adminnarayan/users/${transfer.recipientUsername || transfer.recipientId}`}
                          className="hover:underline font-medium"
                          onClick={(e) => e.stopPropagation()}
                        >
                      ...
                      <TableCell className="font-medium">
                        {transfer.amount.toFixed(8)} {transfer.crypto}
                      </TableCell>
                      <TableCell>
                        {toDate(transfer.createdAt)?.toLocaleString('default', {
                          dateStyle: 'short',
                          timeStyle: 'short',
                        })}
                      </TableCell>
          ...
          <div className="grid gap-4 md:hidden">
            ...
            {!isLoading && !filteredTransfers?.length && (
              <p className="text-center text-sm text-muted-foreground py-8">No transfers found.</p>
            )}
          </div>
        </CardContent>
      </Card>
            ...
            <div className="space-y-4 py-4 text-sm">
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  <span className="font-mono text-xs">{selectedTransfer.id}</span>
                  <Button
                    size="icon"
                    variant="ghost"
                    className="h-6 w-6"
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-medium">{selectedTransfer.senderUsername}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-muted-foreground">Recipient</span>
                <span className="font-medium">{selectedTransfer.recipientUsername}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-muted-foreground">Amount</span>
                <Badge variant="outline">
                  {selectedTransfer.amount.toFixed(8)} {selectedTransfer.crypto}
                </Badge>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-muted-foreground">Date</span>
                <span className="font-medium">
                  {toDate(selectedTransfer.createdAt)?.toLocaleString('default', {
                    dateStyle: 'short',
                    timeStyle: 'short',
                  })}
                </span>
