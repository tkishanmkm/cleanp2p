...
function normalizeAd(raw: any): P2PAd {
  const rawSide = String(raw.type || raw.side || raw.ad_type || raw.adType || 'sell').toLowerCase();
...
function P2PMarketplaceContent() {
  ...
  const isBuyPage = pathname.includes('/buy') || !pathname.includes('/sell');
      ...
      if (creatorIds.length > 0) {
        ...
        if (profiles) {
          ...
          try {
            ...
            console.warn('Wallet balance fetch warning:', wErr);
          }
  ...
  const filteredAds = useMemo(() => {
    ...
    let result = updatedAds.filter(ad => {
      // Filter by adType: /buy shows 'sell' ads (visitors buy crypto), /sell shows 'buy' ads (visitors sell crypto)
      if (ad.adType !== targetAdType) return false;
    ...
    result.sort((a, b) => {
      if (sortBy === 'price') {
          ...
          return isBuyPage ? priceA - priceB : priceB - priceA;
      }
      if (sortBy === 'rating') {
    ...
    return result;
  }, [ads, amount, paymentMethod, selectedCoin, selectedFiat, selectedCountry, showTopRated, showVerifiedOnly, showRecentlyActive, showAcceptable, selectedTags, currentUserData, sortBy, prices, fiatRates, adCreators, isBuyPage]);
                ...
                <div>
                    <div className="flex items-center gap-3">
                      <h1 className="text-2xl font-bold md:text-3xl tracking-tight">
                        {isBuyPage ? 'Buy' : 'Sell'} <span className={cn("font-extrabold", currentCoinInfo.textClass)}>{currentCoinInfo.fullName}</span>
                      </h1>
              ...
              <Card className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100 p-3 shadow-xl transition-colors">
                <div className="flex flex-wrap items-center gap-2.5">
                    ...
                    <div className="flex items-center gap-2 ml-auto">
                        ...
                        >
                            <Link href="/ads/create-buy-ad"><PlusCircle className="mr-2 h-4 w-4" /> Post Offer</Link>
                        </Button>
                        <Button 
                          id="tour-filter-btn"
                          variant="outline" 
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 mt-6">
          ...
          <div className="flex justify-between items-center mb-6 border-b border-border pb-4">
              <div>
                <h2 className="text-xl font-bold text-foreground">
                  Available Offers to {isBuyPage ? 'Buy' : 'Sell'} {currentCoinInfo.fullName}
                </h2>
                <p className="text-xs text-muted-foreground mt-0.5">Showing live competitive rates from verified traders</p>
              </div>
              <div className="flex items-center gap-2">
        ...
        <div className="my-12 bg-card border border-border rounded-xl p-6 sm:p-8 space-y-6 shadow-sm">
            <div className="border-b border-border pb-4">
              <h2 className="text-2xl font-bold text-foreground">
                How to {isBuyPage ? 'Buy' : 'Sell'} {currentCoinInfo.fullName}
              </h2>
              <p className="text-sm text-muted-foreground mt-1">
                Follow our step-by-step guide to trade cryptocurrency safely using Escrow protection.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div className="space-y-2 p-4 rounded-lg bg-muted/40 border border-border">
                ...
                <h3 className="font-semibold text-base text-foreground">Account Setup</h3>
                <p className="text-xs text-muted-foreground leading-relaxed">
                  Create an account or sign in to your existing one. Signing up comes with a free wallet where you can safely store your crypto.
                </p>
              </div>

              <div className="space-y-2 p-4 rounded-lg bg-muted/40 border border-border">
                <span className="text-xs font-bold text-primary bg-primary/10 px-2.5 py-1 rounded-full">Step 2</span>
                <h3 className="font-semibold text-base text-foreground">Search Offers</h3>
                <p className="text-xs text-muted-foreground leading-relaxed">
                  Select a payment method, enter the amount you want to spend in your preferred currency, then hit Search for Offers.
                </p>
              </div>

              <div className="space-y-2 p-4 rounded-lg bg-muted/40 border border-border">
                <span className="text-xs font-bold text-primary bg-primary/10 px-2.5 py-1 rounded-full">Step 3</span>
                <h3 className="font-semibold text-base text-foreground">Review Trader Reputation</h3>
                <p className="text-xs text-muted-foreground leading-relaxed">
                  Go over each offer and review terms and conditions. Keep an eye on seller rates, reputation scores, and feedback.
                </p>
              </div>

              <div className="space-y-2 p-4 rounded-lg bg-muted/40 border border-border">
                <span className="text-xs font-bold text-primary bg-primary/10 px-2.5 py-1 rounded-full">Step 4</span>
                <h3 className="font-semibold text-base text-foreground">Initiate Trade Chat</h3>
                <p className="text-xs text-muted-foreground leading-relaxed">
                  Once you find an offer that fits your requirements, confirm the amount and start the trade to open live chat.
                </p>
              </div>

              <div className="space-y-2 p-4 rounded-lg bg-muted/40 border border-border">
                <span className="text-xs font-bold text-primary bg-primary/10 px-2.5 py-1 rounded-full">Step 5</span>
                <h3 className="font-semibold text-base text-foreground">Make Payment</h3>
                <p className="text-xs text-muted-foreground leading-relaxed">
                  Follow seller instructions carefully and confirm transaction once you complete payment via your payment provider.
                </p>
              </div>

              <div className="space-y-2 p-4 rounded-lg bg-muted/40 border border-border">
                <span className="text-xs font-bold text-primary bg-primary/10 px-2.5 py-1 rounded-full">Step 6</span>
                <h3 className="font-semibold text-base text-foreground">Escrow Release</h3>
                <p className="text-xs text-muted-foreground leading-relaxed">
                  Once payment is validated, seller releases the crypto held safely in Escrow directly into your secure wallet.
                </p>
              </div>
            </div>
        </div>
        ...
        <div className="mb-16 bg-card border border-border rounded-xl p-6 sm:p-8 shadow-sm">
            <div className="mb-6">
              ...
              <p className="text-sm text-muted-foreground mt-1">Find answers to popular questions asked by P2P traders.</p>
            </div>

            <Accordion type="single" collapsible className="w-full space-y-2">
              <AccordionItem value="faq-1" className="border-border">
                <AccordionTrigger className="text-foreground font-semibold">What is P2P crypto trading?</AccordionTrigger>
                <AccordionContent className="text-muted-foreground text-sm leading-relaxed">
                  Peer-to-Peer (P2P) trading allows users to buy and sell cryptocurrencies directly with each other without an intermediary institution.
                </AccordionContent>
              </AccordionItem>
              <AccordionItem value="faq-2" className="border-border">
                <AccordionTrigger className="text-foreground font-semibold">Which cryptocurrencies can I trade?</AccordionTrigger>
                <AccordionContent className="text-muted-foreground text-sm leading-relaxed">
                  You can trade Bitcoin (BTC), Tether (USDT), Ethereum (ETH), and Litecoin (LTC) across hundreds of local fiat payment methods.
                </AccordionContent>
              </AccordionItem>
              <AccordionItem value="faq-3" className="border-border">
                <AccordionTrigger className="text-foreground font-semibold">Is P2P trading safe?</AccordionTrigger>
                <AccordionContent className="text-muted-foreground text-sm leading-relaxed">
                  Yes, every transaction is backed by secure Escrow protection. Crypto is locked in Escrow when a trade starts.
                </AccordionContent>
              </AccordionItem>
            </Accordion>
        </div>
