...
export default function ProfilePage() {
  ...
  const loadProfileData = useCallback(async () => {
    ...
    try {
      ...
      const stats = {
        total_volume: st?.total_trade_volume || 0,
        completed_trades: st?.completed_trades || profile?.completed_trades || 0,
        avg_payment_time: Math.round((st?.avg_payment_seconds || 240) / 60),
        avg_release_time: Math.round((st?.avg_release_seconds || 120) / 60),
      };
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-6 bg-white dark:bg-[#0f1423] border border-slate-200 dark:border-[#1e2640] rounded-3xl gap-4 shadow-xl">
        ...
        <div className="flex items-center gap-2 text-xs text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-3.5 py-2 rounded-xl border border-emerald-500/20 font-medium">
