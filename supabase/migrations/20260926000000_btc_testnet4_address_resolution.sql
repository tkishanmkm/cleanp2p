-- Migration: 20260926000000_btc_testnet4_address_resolution.sql
-- Description: Updates public.process_deposit_atomic() to support Bitcoin Testnet4 network identity mapping and case-insensitive address resolution
...
BEGIN
    ...
    -- Normalized address strictly matching canonical functional index (Case-insensitive for EVM and Bech32 BTC/LTC)
    v_norm_address := CASE 
        WHEN v_norm_network IN ('ETH', 'ERC20', 'BEP20', 'BTC', 'LTC') THEN LOWER(BTRIM(p_destination_address))
    ...
    ELSIF v_norm_network = 'ERC20' AND v_norm_asset = 'USDT' THEN
        IF LOWER(v_norm_contract) NOT IN (
            ...
            '0x7169d38820dfd117c3fa1f22a697dba58d90ba06', -- Sepolia Testnet USDT (Canonical)
            '0xaa8e23fb1079ea71e0a56f48a2aa51851d8433d0'  -- Sepolia Testnet USDT (Legacy)
        ) THEN
    ...
    WHERE (
        CASE
            WHEN UPPER(BTRIM(COALESCE(network_code, network::text)))
                 IN ('ETH', 'ERC20', 'BEP20', 'ETHEREUM', 'BTC', 'BITCOIN', 'BITCOIN_TESTNET4', 'BTC_TESTNET4', 'LTC', 'LITECOIN')
    ...
    AND (
        CASE
            WHEN UPPER(BTRIM(COALESCE(network_code, network::text))) = 'ETHEREUM' AND v_norm_network = 'ERC20' THEN 'ERC20'
            WHEN UPPER(BTRIM(COALESCE(network_code, network::text))) = 'ETHEREUM' THEN 'ETH'
            WHEN UPPER(BTRIM(COALESCE(network_code, network::text))) IN ('BITCOIN', 'BITCOIN_TESTNET4', 'BTC_TESTNET4') THEN 'BTC'
            WHEN UPPER(BTRIM(COALESCE(network_code, network::text))) = 'LITECOIN' THEN 'LTC'
            ELSE UPPER(BTRIM(COALESCE(network_code, network::text)))
        END
    ) = v_norm_network
    LIMIT 1;
