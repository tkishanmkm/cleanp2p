-- ============================================================
-- MIGRATION: 20260924000000_canonical_deposit_rpc_v2.sql
-- TARGET: paxonesflows (ref: eaiwgfxoiwxepinvcykg)
-- MODE: ATOMIC TRANSACTIONAL MIGRATION
-- DESCRIPTION: Canonical Deposit Architecture (Model B) against Live Schema
-- ============================================================

BEGIN;

-- ------------------------------------------------------------
-- STEP 1: PREFLIGHT SCHEMA AUDIT GUARD
-- ------------------------------------------------------------
DO $$
DECLARE
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'wallets') THEN
        v_missing_tables := array_append(v_missing_tables, 'wallets');
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'wallet_assets') THEN
        v_missing_tables := array_append(v_missing_tables, 'wallet_assets');
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'deposits') THEN
        v_missing_tables := array_append(v_missing_tables, 'deposits');
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'onchain_deposits') THEN
        v_missing_tables := array_append(v_missing_tables, 'onchain_deposits');
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_deposit_addresses') THEN
        v_missing_tables := array_append(v_missing_tables, 'user_deposit_addresses');
    END IF;

    IF array_length(v_missing_tables, 1) > 0 THEN
        RAISE EXCEPTION 'MIGRATION ABORTED: Missing required database tables: %', array_to_string(v_missing_tables, ', ');
    END IF;
END $$;

-- ------------------------------------------------------------
-- STEP 2: HISTORICAL-ROW SAFETY GATE
-- ------------------------------------------------------------
DO $$
DECLARE
    v_onchain_count BIGINT;
    v_deposits_count BIGINT;
BEGIN
    SELECT COUNT(*) INTO v_onchain_count FROM public.onchain_deposits;
    SELECT COUNT(*) INTO v_deposits_count FROM public.deposits;

    IF v_onchain_count > 0 OR v_deposits_count > 0 THEN
        RAISE EXCEPTION 'MIGRATION ABORTED: Historical records detected in onchain_deposits (%) or deposits (%). Backfill/reconciliation required before applying canonical schema changes.', v_onchain_count, v_deposits_count;
    END IF;
END $$;

-- ------------------------------------------------------------
-- STEP 3: SCHEMA EXTENSION & INDEX RESTRUCTURING
-- ------------------------------------------------------------
-- 3A. Add output_index column to onchain_deposits if missing
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema = 'public' AND table_name = 'onchain_deposits' AND column_name = 'output_index'
    ) THEN
        ALTER TABLE public.onchain_deposits ADD COLUMN output_index INTEGER NOT NULL DEFAULT 0;
    END IF;
END $$;

-- 3B. Safely drop legacy single-column unique constraint on onchain_deposits.tx_hash
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conrelid = 'public.onchain_deposits'::regclass AND conname = 'onchain_deposits_tx_hash_key'
    ) THEN
        ALTER TABLE public.onchain_deposits DROP CONSTRAINT onchain_deposits_tx_hash_key;
    END IF;
END $$;

-- 3C. Create composite unique index on onchain_deposits (LOWER(BTRIM(tx_hash)), UPPER(BTRIM(network)), output_index)
CREATE UNIQUE INDEX IF NOT EXISTS uq_onchain_deposits_tx_net_out 
ON public.onchain_deposits (LOWER(BTRIM(tx_hash)), UPPER(BTRIM(network)), output_index);

-- 3D. Create composite unique index on deposits (LOWER(BTRIM(txid)), UPPER(BTRIM(network_code)), output_index)
CREATE UNIQUE INDEX IF NOT EXISTS uq_deposits_tx_net_out 
ON public.deposits (LOWER(BTRIM(txid)), UPPER(BTRIM(network_code)), output_index);

-- 3E. Safely replace legacy user_deposit_addresses constraint with canonical functional unique index
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conrelid = 'public.user_deposit_addresses'::regclass AND conname = 'uq_user_deposit_addresses_addr_net'
    ) THEN
        ALTER TABLE public.user_deposit_addresses DROP CONSTRAINT uq_user_deposit_addresses_addr_net;
    END IF;
END $$;

CREATE UNIQUE INDEX IF NOT EXISTS uq_user_deposit_addresses_canonical 
...
-- ------------------------------------------------------------
-- STEP 4: CREATE CANONICAL PROCESS_DEPOSIT_ATOMIC RPC
-- ------------------------------------------------------------
...
BEGIN
    ...
    ELSIF v_norm_network = 'ERC20' AND v_norm_asset = 'USDT' THEN
        IF LOWER(v_norm_contract) <> '0xdac17f958d2ee523a2206206994597c13d831ec7' THEN
    ...
    ELSIF v_norm_network = 'BEP20' AND v_norm_asset = 'USDT' THEN
        IF LOWER(v_norm_contract) <> '0x55d398326f99059ff775485246999027b3197955' THEN
    ...
    ELSIF v_norm_network = 'TRC20' AND v_norm_asset = 'USDT' THEN
        IF v_norm_contract <> 'TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t' THEN
    ...
    -- Lock and verify wallet_assets provisioning (fail closed if missing)
    ...
    IF NOT FOUND THEN
        RETURN jsonb_build_object(
            'success', false, 
            'code', 'WALLET_ASSET_NOT_PROVISIONED', 
            'message', 'Wallet asset balance record is not provisioned for this user and asset'
        );
    END IF;
