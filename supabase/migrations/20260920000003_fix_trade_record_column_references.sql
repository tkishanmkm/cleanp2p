-- ==============================================================================
-- 20260920000003_fix_trade_record_column_references.sql
-- Fixes PostgreSQL Error: record v_trade has no field "asset_symbol"
-- public.trades schema has columns: crypto, asset, crypto_currency, asset_code, coin,
-- crypto_amount, amount, amount_crypto, escrow_fee, fee_crypto.
-- ==============================================================================
...
-- ==============================================================================
-- 2. CANCEL P2P TRADE RPC (Strict Column Mapping)
-- ==============================================================================
...
BEGIN
  ...
  -- 6. Refund locked escrow to seller across all wallet / balance tables using explicit cast
  IF v_trade.seller_id IS NOT NULL AND v_total_refund > 0 THEN
    ...
    SET available_balance = COALESCE(available_balance, balance, 0) + v_total_refund,
        ...
        updated_at = NOW()
    WHERE user_id::TEXT = v_trade.seller_id::TEXT AND UPPER(asset_symbol::text) = v_asset_code;
    ...
    SET available = COALESCE(available, balance, 0) + v_total_refund,
        locked_balance = GREATEST(0, COALESCE(locked_balance, 0) - v_total_refund),
        reserved_balance = GREATEST(0, COALESCE(reserved_balance, 0) - v_total_refund),
        in_escrow = GREATEST(0, COALESCE(in_escrow, 0) - v_total_refund),
        updated_at = NOW()
    WHERE user_id::TEXT = v_trade.seller_id::TEXT AND UPPER(asset_symbol::text) = v_asset_code;
    ...
    SET available_balance = available_balance + v_total_refund,
        ...
        reserved_balance = GREATEST(0, reserved_balance - v_total_refund),
        updated_at = NOW()
    WHERE user_id::TEXT = v_trade.seller_id::TEXT AND UPPER(currency::text) = v_asset_code;
    ...
    SET available_balance = available_balance + v_total_refund,
        locked_balance = GREATEST(0, locked_balance - v_total_refund),
        updated_at = NOW()
    WHERE user_id::TEXT = v_trade.seller_id::TEXT AND UPPER(asset::text) = v_asset_code;
...
-- ==============================================================================
-- 3. RELEASE P2P ESCROW RPC (Strict Column Mapping)
-- ==============================================================================
...
BEGIN
  ...
  -- Safe conversion of buyer_id to UUID
  BEGIN
  ...
  IF v_trade.seller_id IS NOT NULL AND v_total_deduct > 0 THEN
    ...
    SET locked_balance = GREATEST(0, COALESCE(locked_balance, 0) - v_total_deduct),
        reserved_balance = GREATEST(0, COALESCE(reserved_balance, 0) - v_total_deduct),
        in_escrow = GREATEST(0, COALESCE(in_escrow, 0) - v_total_deduct),
        updated_at = NOW()
    WHERE user_id::TEXT = v_trade.seller_id::TEXT AND UPPER(asset_symbol::text) = v_asset_code;
    ...
    SET locked_balance = GREATEST(0, locked_balance - v_total_deduct),
        reserved_balance = GREATEST(0, reserved_balance - v_total_deduct),
  ...
  IF v_trade.buyer_id IS NOT NULL AND v_coin_amount > 0 AND v_buyer_uuid IS NOT NULL THEN
    ...
    SET available_balance = COALESCE(available_balance, balance, 0) + v_coin_amount,
        ...
        updated_at = NOW()
    WHERE user_id::TEXT = v_trade.buyer_id::TEXT AND UPPER(asset_symbol::text) = v_asset_code;

    -- Table: wallet_assets
    INSERT INTO public.wallet_assets (user_id, asset_symbol, available, locked_balance, in_escrow, updated_at)
...
-- ==============================================================================
-- 4. EXPIRE P2P TRADE RPC (Strict Column Mapping)
-- ==============================================================================
...
BEGIN
  ...
  IF v_trade.seller_id IS NOT NULL AND v_total_refund > 0 THEN
    ...
    WHERE user_id::TEXT = v_trade.seller_id::TEXT
      AND UPPER(asset_symbol::text) = v_crypto_sym;
    ...
    SET available_balance = COALESCE(available_balance, 0) + v_total_refund,
        ...
        reserved_balance = GREATEST(0, COALESCE(reserved_balance, 0) - v_total_refund),
        updated_at = NOW()
    WHERE user_id::TEXT = v_trade.seller_id::TEXT AND UPPER(asset_symbol::text) = v_crypto_sym;
    ...
    SET available_balance = available_balance + v_total_refund,
        ...
        reserved_balance = GREATEST(0, reserved_balance - v_total_refund),
        updated_at = NOW()
    WHERE user_id::TEXT = v_trade.seller_id::TEXT AND UPPER(currency::text) = v_crypto_sym;

    UPDATE public.balances
    SET available_balance = COALESCE(available_balance, 0) + v_total_refund,
        locked_balance = GREATEST(0, COALESCE(locked_balance, 0) - v_total_refund),
        updated_at = NOW()
    WHERE user_id::TEXT = v_trade.seller_id::TEXT AND UPPER(asset::text) = v_crypto_sym;
...
-- ==============================================================================
-- 5. CANCEL EXPIRED P2P TRADES BATCH RUNNER (Strict Column Mapping)
-- ==============================================================================
