-- ==============================================================================
-- 20260920000005_fix_wallet_assets_column_error.sql
-- Fixes PostgreSQL Error: column "wallet_id" does not exist in public.wallet_assets
--
-- Schema Design:
-- 1. `public.wallet_assets` uses `user_id` (UUID/TEXT) and `asset_symbol` (TEXT).
-- 2. Balances are tracked via `balance`, `locked_balance`, `available`, `locked_escrow`.
-- 3. `public.user_balances` tracks `user_id`, `asset`, `available_balance`, `locked_balance`.
-- ==============================================================================

-- 1. Cleanly drop any existing function signatures
DO $$
DECLARE
    r RECORD;
BEGIN
...
BEGIN
  ...
  -- 1. Fetch trade record with lock
  SELECT * INTO v_trade
  FROM public.trades
  WHERE id = p_trade_id
  FOR UPDATE;
  ...
  -- 2. Authorization check with explicit TEXT conversions
  ...
  -- 6. Refund locked escrow to seller in public.wallet_assets (keyed by user_id & asset_symbol)
  IF v_trade.seller_id IS NOT NULL AND v_total_refund > 0 THEN
    -- Update wallet_assets table safely
    BEGIN
      UPDATE public.wallet_assets
      SET balance = COALESCE(balance, 0) + v_total_refund,
          locked_balance = GREATEST(0, COALESCE(locked_balance, 0) - v_total_refund),
          updated_at = NOW()
      WHERE user_id::TEXT = v_trade.seller_id::TEXT 
        AND UPPER(COALESCE(asset_symbol, '')) = v_asset_code;
    EXCEPTION WHEN OTHERS THEN
      NULL;
    END;

    -- Also update available/locked_escrow if those column variations exist
    BEGIN
      UPDATE public.wallet_assets
      SET available = COALESCE(available, 0) + v_total_refund,
    ...
    -- Also update user_balances table if present
    BEGIN
      ...
      SET available_balance = available_balance + v_total_refund,
          ...
          updated_at = NOW()
      WHERE user_id::TEXT = v_trade.seller_id::TEXT 
        AND UPPER(COALESCE(asset, asset_symbol, '')) = v_asset_code;
    EXCEPTION WHEN OTHERS THEN
      NULL;
    END;

    -- Also update user_wallets table if present
    BEGIN
...
BEGIN
  ...
  IF v_trade.buyer_id IS NOT NULL AND v_coin_amount > 0 THEN
    BEGIN
      IF EXISTS (SELECT 1 FROM public.wallet_assets WHERE user_id::TEXT = v_trade.buyer_id::TEXT AND UPPER(COALESCE(asset_symbol, '')) = v_asset_code) THEN
        UPDATE public.wallet_assets
        SET balance = COALESCE(balance, 0) + v_coin_amount,
            updated_at = NOW()
        WHERE user_id::TEXT = v_trade.buyer_id::TEXT AND UPPER(COALESCE(asset_symbol, '')) = v_asset_code;
      ELSIF v_buyer_uuid IS NOT NULL THEN
        INSERT INTO public.wallet_assets (user_id, asset_symbol, balance, locked_balance, updated_at)
        VALUES (v_buyer_uuid, v_asset_code, v_coin_amount, 0, NOW());
      END IF;
    EXCEPTION WHEN OTHERS THEN
      NULL;
    END;
    ...
    BEGIN
      UPDATE public.wallet_assets
      SET available = COALESCE(available, 0) + v_coin_amount,
          updated_at = NOW()
      WHERE user_id::TEXT = v_trade.buyer_id::TEXT AND UPPER(COALESCE(asset_symbol, '')) = v_asset_code;
    EXCEPTION WHEN OTHERS THEN
      NULL;
  ...
  IF v_trade.seller_id IS NOT NULL AND v_total_refund > 0 THEN
    BEGIN
      ...
      SET balance = COALESCE(balance, 0) + v_total_refund,
          ...
          updated_at = NOW()
      WHERE user_id::TEXT = v_trade.seller_id::TEXT 
        AND UPPER(COALESCE(asset_symbol, '')) = v_crypto_sym;
    EXCEPTION WHEN OTHERS THEN
      NULL;
    END;
    ...
    BEGIN
      ...
      SET available_balance = available_balance + v_total_refund,
          ...
          updated_at = NOW()
      WHERE user_id::TEXT = v_trade.seller_id::TEXT 
        AND UPPER(COALESCE(asset, asset_symbol, '')) = v_crypto_sym;
    EXCEPTION WHEN OTHERS THEN
      NULL;
    END;
  END IF;
...
-- ==============================================================================
-- 5. CANCEL EXPIRED P2P TRADES BATCH RUNNER
-- ==============================================================================
