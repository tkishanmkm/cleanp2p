-- ==============================================================================
-- Migration: 20260919050000_fix_enum_trade_status_buyer_marked_paid.sql
-- Purpose: Permanently resolve PostgreSQL Error 22P02:
--          "invalid input value for enum trade_status: 'buyer_marked_paid'"
--          and ensure seamless trade initiation, payment marking, & completion.
-- ==============================================================================

-- 1. If trade_status enum exists, add all missing enum values safely
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'trade_status') THEN
    ...
    END;
    BEGIN
      ALTER TYPE trade_status ADD VALUE IF NOT EXISTS 'created';
    EXCEPTION WHEN duplicate_object THEN NULL; WHEN OTHERS THEN NULL;
    END;
    BEGIN
      ALTER TYPE trade_status ADD VALUE IF NOT EXISTS 'CREATED';
...
-- 3. Relax and modernize public.trades status column
DO $$
BEGIN
  -- Drop restrictive check constraints on trades.status if any exist
  IF EXISTS (
  ...
  -- Ensure status is standard TEXT to prevent strict enum comparison bottlenecks
  BEGIN
...
-- 4. Update fn_update_user_trade_metrics trigger function with defensive text casting
