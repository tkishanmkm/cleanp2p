-- ============================================================================
-- Supabase Migration: 20260925000009_harden_persist_signed_evm_transaction.sql
-- Description: Blocker #4 Fix - State Machine Hardening for persist_signed_evm_transaction
--              Restricts source state transitions strictly to:
--              CLAIMED, GAS_FUNDING, and GAS_FUNDED.
--              Rejects all other statuses (PENDING, READY_TO_BROADCAST, BROADCASTED, etc.)
-- ============================================================================
...
BEGIN
    ...
    -- 4. Verify valid source status (ONLY CLAIMED, GAS_FUNDING, GAS_FUNDED allowed)
    IF v_sweep.status NOT IN ('CLAIMED', 'GAS_FUNDING', 'GAS_FUNDED') THEN
        RETURN jsonb_build_object(
            'success', false, 
            'code', 'INVALID_SOURCE_STATE', 
            'message', 'Signed transaction persistence is only allowed for sweeps in CLAIMED, GAS_FUNDING, or GAS_FUNDED states. Current state: ' || v_sweep.status
        );
    END IF;
    ...
    -- 7. Require EVM-compatible network
    ...
    -- 8. Handle nonce matching & validation
    ...
    -- 9. Reject overwriting existing different signed_tx_hex
    ...
    -- 10. Reject overwriting existing different sweep_tx_hash
    ...
    -- 11. Atomically update status to READY_TO_BROADCAST and save signed bytes & hash
...
-- 12. Security Hardening: Revoke PUBLIC / anon / authenticated permissions
